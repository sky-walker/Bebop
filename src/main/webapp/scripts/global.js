...
var send_msg_submit = function (v, h, f) {
    if (v == 'ok') {
        ...
        ajax_post_json("/messages/private/dosend",{nickname:fname,content:content},function(data) {
			if(data.status) {
				$.jBox.tip("消息发送成功!","success",{timeout:800});
			}else {
...
function ajax_box_login() {
	$.jBox.tip("进行登录...", 'loading',{top:'135px'});
	$(this).attr("disabled",true);
	$(this).attr("value","正在提交登录信息...");
	var email = $("#ajax-login-box input[id=email]").val();
	var password = $("#ajax-login-box input[id=password]").val();
	var vcode = $("#ajax-login-box input[id=vcode]").val();
	var rememberme = $("#ajax-login-box input[id=rememberme]").val();
    ajax_post_json("/user/loginactive",{email:email,pwd:password,vcode:vcode,rememberme:rememberme},function(data) {
		if(data.status) {
			$.jBox.tip("登录成功!",'success');
			window.setTimeout(function () { location.reload(); }, 500);
		}else {
			$.jBox.tip(data.msg,'info',{top:'135px',timeout:500});
			$('#ajax-login-btn').attr("disabled",false);
			$('#ajax-login-btn').attr("value","现在登录");
			return false;
		}
	});
}
